...
def _compile_results(gt, bboxes, dist_threshold):
    ...
    thresholds = np.linspace(0, 50, n_thresholds+1)
    ...
    precision_auc = np.trapz(precisions_ths,thresholds)    

    # per frame averaged intersection over union (OTB metric)
    n_thresholds_ious = 20
    thresholds = np.linspace(0, 1, n_thresholds_ious)
    ious_ths = np.zeros(n_thresholds_ious)
    for i in range(n_thresholds_ious):
        ious_ths[i] = sum(new_ious > thresholds[i])/np.size(new_ious)
    # integrate over the thresholds
    # success_auc = np.trapz(ious_ths, thresholds)
    success_auc = np.mean(ious_ths)
    iou = np.mean(new_ious) * 100

    return l, precision, precision_auc, iou, success_auc
...
def _init_video(env, evaluation, video):
    ...
    return gt, frame_name_list, frame_sz, n_frames, img_mode

def _init_video_OTB(env, evaluation, video):
    video_folder = os.path.join(env.root_dataset, evaluation.dataset, video)
    frame_name_list = [f for f in os.listdir(video_folder+'/img') if f.endswith(".jpg")]
    frame_name_list = [os.path.join(env.root_dataset, evaluation.dataset, video, 'img', '') + s for s in frame_name_list]
    frame_name_list.sort()
    with Image.open(frame_name_list[0]) as img:
        frame_sz = np.asarray(img.size)
        frame_sz[1], frame_sz[0] = frame_sz[0], frame_sz[1]

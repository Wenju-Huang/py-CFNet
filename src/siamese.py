...
_pool_stride = np.array([2,0]) # 0 means no pool
_pool_sz = 3
_bnorm_adjust = False

# cfnet-conv 1
'''
_conv_stride = np.array([4])
_filtergroup_yn = np.array([0], dtype=bool)
_bnorm_yn = np.array([1], dtype=bool)
_relu_yn = np.array([1], dtype=bool)
_pool_stride = np.array([0]) # 0 means no pool
_pool_sz = 3
_bnorm_adjust = False
'''
...
def build_tracking_graph(final_score_sz, design, env):
    ...
    image =  tf.cond(tf.less(frame_sz[2], tf.constant(3)), lambda: tf.concat([image, image, image],2), lambda:image)
    '''
    image = tf.placeholder(tf.float32, [None,None,3], name='filename')
    ...
    scores = _match_templates_cfnet(templates_z, templates_x, p_names_list, p_val_list, design.exemplar_corp_pad)
...
def _match_templates_cfnet(net_z, net_x, params_names_list, params_values_list, corp_pad):
    # finalize network
    join_window = params_values_list[params_names_list.index('join_window')]
    cf_target = params_values_list[params_names_list.index('join_cf_target')]
    join_window = tf.expand_dims(join_window,2)
    net_z = net_z[0,:,:,:]
    net_z = tf.multiply(net_z, join_window, 'join_cf_window')
    
    # net_z are [H, W, C]
    net_z= tf.transpose(net_z, perm=[2,0,1])
    # net_z are [C, H, W]
    #tf.fft2d computes the 2-dimensional discrete Fourier transform over the inner-most 2 dimensions of input  
    net_z_f = tf.fft2d(tf.cast(net_z, tf.complex64), 'fft_z')
    # net_z, net_z_f are [C, H, W]
    net_z= tf.transpose(net_z, perm=[1,2,0])
    net_z_f= tf.transpose(net_z_f, perm=[1,2,0])
    # net_z, net_z_f are [H, W, C]
    
    # cf_target are [H, W]
    cf_target_f = tf.fft2d(tf.cast(cf_target, tf.complex64), 'fft_target')
    
    H, W, C = tf.unstack(tf.shape(net_z))
    ...
    #net_w = tf.image.central_crop(net_w, 17.0/49.0)
    net_w = tf.image.crop_to_bounding_box(net_w, corp_pad, corp_pad, H-2*corp_pad,W-2*corp_pad)

    net_w = tf.stack([net_w, net_w, net_w])

    # z, x are [B, H, W, C]
    net_w = tf.transpose(net_w, perm=[1,2,0,3])
    net_x = tf.transpose(net_x, perm=[1,2,0,3])
    # z, x are [H, W, B, C]
    Hz, Wz, B, C = tf.unstack(tf.shape(net_w))
    Hx, Wx, Bx, Cx = tf.unstack(tf.shape(net_x))
    # assert B==Bx, ('Z and X should have same Batch size')
    # assert C==Cx, ('Z and X should have same Channels number')
    net_w = tf.reshape(net_w, (Hz, Wz, B*C, 1))
    net_x = tf.reshape(net_x, (1, Hx, Wx, B*C))

    net_final = tf.nn.depthwise_conv2d(net_x, net_w, strides=[1,1,1,1], padding='VALID')
    # final is [1, Hf, Wf, BC]
    net_final = tf.concat(tf.split(net_final, 3, axis=3), axis=0)
    # final is [B, Hf, Wf, C]
    net_final = tf.expand_dims(tf.reduce_sum(net_final, axis=3), axis=3)
    ...
    '''
        ...
        fin_adjust_s = params_values_list[params_names_list.index('fin_adjustf')]
        fin_adjust_b = params_values_list[params_names_list.index('fin_adjustb')]
        net_final = net_final * fin_adjust_s + fin_adjust_b
    '''
    return net_final

...
def tracker(hp, run, design, frame_name_list, pos_x, pos_y, target_w, target_h, final_score_sz, image, templates_z, scores, start_frame):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        image_plt = plt.imread(frame_name_list[0])
        if len(np.shape(image_plt)) < 3:
            image_plt = np.stack([image_plt, image_plt, image_plt], 2)
        ...
        for i in range(1, num_frames):        
            ...
            image_plt = plt.imread(frame_name_list[i])
            if len(np.shape(image_plt)) < 3:
                image_plt = np.stack([image_plt, image_plt, image_plt], 2)
            ...
            x_sz = max(min_x, min(max_x, (1-hp.scale_lr)*x_sz + hp.scale_lr*scaled_search_area[new_scale_id]))       
            ...
            z_sz = max(min_z, min(max_z,(1-hp.scale_lr)*z_sz + hp.scale_lr*scaled_exemplar[new_scale_id]))
            
            if run.visualization:
                show_frame(image_plt, bboxes[i,:], 1)
